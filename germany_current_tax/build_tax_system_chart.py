...
ROOT_NAME = "All German taxes"
LABEL_THRESHOLD_EUR = 250_000_000
VISUAL_PAD_THRESHOLD_EUR = 1_000_000_000
VISUAL_PAD_MAX_DEPTH = 5
...
ITEM_COLORS = {
    # Keep high-profile analogous slices visually consistent with the France chart.
    "VAT": "#F76707",
    "Social contributions": "#3FBAC2",
    "Personal income taxes": "#0A9396",
    "Profit and sector taxes": "#1E3A5F",
    "Recurrent land and property taxes": "#B7791F",
    "Inheritance and gifts": "#5B21B6",
    "Carbon and emissions taxes": "#1B5E20",
}
...
FORCE_LABELS = {
    ...
    "Energy and fuel duties",
    "Import duties",
    "Financial capital taxes",
    "Carbon and emissions taxes",
}
...
def pad_small_terminal_nodes(items, depth=1, max_depth=VISUAL_PAD_MAX_DEPTH):
    for item in items:
        if "children" in item:
            pad_small_terminal_nodes(item["children"], depth + 1, max_depth)
            continue
        if depth >= max_depth or item["actual_eur"] > VISUAL_PAD_THRESHOLD_EUR:
            continue
        child = {
            "name": item["name"],
            "value": item["value"],
            "actual_eur": item["actual_eur"],
...
def make_tooltip(item, mode, total_eur=None):
    ...
    if local_name and normalise_name(local_name) not in english_names:
        details.append(f"<em>{html.escape(local_name)}</em>")
...
def apply_colors(items, palette):
    for idx, item in enumerate(items):
        color = ITEM_COLORS.get(item["name"], TOP_COLOR_BY_LAYER.get(item["name"], palette[idx % len(palette)]))
...
def build_option():
    ...
    # Visual-only: extend tiny terminal slices radially with hidden same-value
    # children. This does not change their angle, totals or tooltip values.
    pad_small_terminal_nodes(data)
